...
func (counter *ProgressCounter) PrintProgress() {
	...
	if ratio%5 == 0 && ratio != counter.PreviousRatio {
		...
		fmt.Printf("\rDownloading... %s out of %s (%d%%)", bytesToHuman(counter.Downloaded), bytesToHuman(counter.Total), ratio)
...
func downloadFile(filepath string, url string) error {
	...
	counter := &ProgressCounter{Total: size, PreviousRatio: 0}
